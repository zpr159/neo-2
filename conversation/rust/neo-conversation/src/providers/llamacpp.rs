...
impl LanguageEngine for LlamaCppProvider {
    fn info(&self) -> LanguageEngineInfo {
        LanguageEngineInfo {
            backend_type: LanguageBackendType::LlamaCpp,
            model: self
                .config
                .as_ref()
    ...
    async fn initialize(&mut self, config: &LanguageEngineConfig) -> ConversationResult<()> {
        ...
        tracing::info!(
            "LlamaCpp provider initialized: model={}, url={}",
            config.model,
            config.base_url
        );
        Ok(())
    ...
    async fn generate(&self, request: &GenerateRequest) -> ConversationResult<GenerateResponse> {
        ...
        if !status.is_success() {
            return Err(ConversationError::LanguageEngineError(format!(
                "LlamaCpp returned status {status}: {text}"
            )));
        }
    ...
    async fn shutdown(&mut self) -> ConversationResult<()> {
        self.client = None;
        self.config = None;
        tracing::info!("LlamaCpp provider shut down");
        Ok(())
    }
}
