...
impl InferenceBackend for OpenVinoBackend {
    ...
    async fn initialize(&mut self, config: &BackendConfig) -> InferenceResult<()> {
        if let Some(v) = config.config.get("device_type").and_then(|v| v.as_str()) {
            self.device_type = v.to_string();
        }
        if let Some(v) = config.config.get("nstreams").and_then(|v| v.as_u64()) {
            self.nstreams = v as usize;
        }
        tracing::info!(device = %self.device_type, nstreams = self.nstreams, "OpenVINO backend initialized");
        Ok(())
    }

    async fn shutdown(&mut self) -> InferenceResult<()> {
        self.loaded_models.clear();
        tracing::info!("OpenVINO backend shutdown");
        Ok(())
    }
    ...
    async fn load_model(&mut self, metadata: &ModelMetadata) -> InferenceResult<ModelId> {
        ...
        tracing::info!(model_id = %model_id, name = %metadata.name, "Model loaded on OpenVINO backend");
        Ok(model_id)
    }
    ...
    async fn unload_model(&mut self, model_id: ModelId) -> InferenceResult<()> {
        ...
        tracing::info!(model_id = %model_id, "Model unloaded from OpenVINO backend");
        Ok(())
    }
    ...
    async fn inference(&self, model_id: ModelId, input: InferenceInput) -> InferenceResult<InferenceOutput> {
        ...
        let mut rng_state: u64 = model_id.0.as_bytes().iter().fold(0u64, |acc, &b| acc.wrapping_mul(31).wrapping_add(b as u64)).wrapping_add(3333);
        let (logits, past_kv) = Self::ov_infer_request(&input.input_ids, metadata, &input.past_key_values, &mut rng_state)?;
        Ok(InferenceOutput {
            logits,
    ...
    fn supported_formats(&self) -> Vec<ModelFormat> { vec![ModelFormat::OpenVino] }
}
