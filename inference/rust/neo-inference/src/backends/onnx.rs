...
impl OnnxBackend {
    ...
    fn onnx_graph_execution(
        ...
        for _layer in 0..num_layers {
            let mut layer_normed = hidden.clone();
            for chunk in layer_normed.chunks_mut(hidden_size) {
                ...
                for i in 0..seq_len {
                    ...
                    for j in 0..seq_len {
                        ...
                        for d in 0..head_dim {
                            ...
                            if qi < layer_normed.len() && kj < layer_normed.len() {
                                dot += layer_normed[qi] * layer_normed[kj];
                            }
                        }
                        scores[j] = dot * scale;
                    }
                    ...
                    for d in 0..head_dim {
                        ...
                        for j in 0..seq_len {
                            ...
                            if vi < layer_normed.len() { val += scores[j] * layer_normed[vi]; }
                        }
            ...
            for chunk in ffn_in.chunks_mut(hidden_size) {
                ...
                for v in chunk.iter_mut() {
                    let n = (*v - mean) * si;
                    *v = 0.5 * n * (1.0 + (1.5915494 * (n + 0.044715 * n.powi(3))).tanh());
                }
            }
        ...
        for _ in new_pasts.len()..num_layers {
            let kv_len = seq_len * num_heads * head_dim;
            let pk: Vec<f32> = (0..kv_len).map(|_| next_random(0.01)).collect();
            let pv: Vec<f32> = (0..kv_len).map(|_| next_random(0.01)).collect();
...
impl InferenceBackend for OnnxBackend {
    ...
    async fn initialize(&mut self, config: &BackendConfig) -> InferenceResult<()> {
        if let Some(v) = config.config.get("intra_op_threads").and_then(|v| v.as_u64()) {
            self.intra_op_threads = v as usize;
        }
        if let Some(v) = config.config.get("inter_op_threads").and_then(|v| v.as_u64()) {
            self.inter_op_threads = v as usize;
        }
        tracing::info!(intra = self.intra_op_threads, inter = self.inter_op_threads, mode = %self.execution_mode, "ONNX backend initialized");
        Ok(())
    }

    async fn shutdown(&mut self) -> InferenceResult<()> {
        self.loaded_models.clear();
        tracing::info!("ONNX backend shutdown");
        Ok(())
    }
    ...
    async fn load_model(&mut self, metadata: &ModelMetadata) -> InferenceResult<ModelId> {
        ...
        tracing::info!(model_id = %model_id, name = %metadata.name, "Model loaded on ONNX backend");
        Ok(model_id)
    }
    ...
    async fn unload_model(&mut self, model_id: ModelId) -> InferenceResult<()> {
        ...
        tracing::info!(model_id = %model_id, "Model unloaded from ONNX backend");
        Ok(())
    }
    ...
    async fn inference(&self, model_id: ModelId, input: InferenceInput) -> InferenceResult<InferenceOutput> {
        ...
        let mut rng_state: u64 = model_id.0.as_bytes().iter().fold(0u64, |acc, &b| acc.wrapping_mul(31).wrapping_add(b as u64)).wrapping_add(11115);
        let (logits, past_kv) = Self::onnx_graph_execution(&input.input_ids, metadata, &input.past_key_values, &mut rng_state)?;
        Ok(InferenceOutput {
            logits,
    ...
    fn supported_formats(&self) -> Vec<ModelFormat> { vec![ModelFormat::Onnx] }
}
